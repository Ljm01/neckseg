...
from dataset.dataset_skull_val import Val_Dataset
from dataset.dataset_skull_train import Train_Dataset
...
if __name__ == '__main__':
    ...
    save_path = r'D:\bishe\ex\trainingrecords'   # 保存训练记录的地址
    os.makedirs(save_path, exist_ok=True)
    device = torch.device('cuda')
    # data info
    dataset_images_train = glob.glob(r'D:\bishe\ex\traing data\image-*-reformed-cut-rz.mhd') # 训练集的数据地址
    random.shuffle(dataset_images_train)
    print('train:', dataset_images_train)
    dataset_images_val = glob.glob(r'D:\bishe\ex\traing data\image-*-reformed-cut-rz.mhd')# 验证集的数据地址
    print('val:', dataset_images_val)
    train_loader = DataLoader(dataset=Train_Dataset(dataset_images_train), batch_size=4,
                              num_workers=0, shuffle=True)
    val_loader = DataLoader(dataset=Val_Dataset(dataset_images_val), batch_size=4,
    ...
    model.apply(weights_init.init_model)  # 模型权重初始化，正态分布
